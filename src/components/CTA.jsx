...
export default function CTA() {
  ...
  return (
    <section id="contact" className="relative py-24 border-t border-border overflow-hidden" ref={ref}>
      ...
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 60% 50% at 50% 50%, rgba(196,164,106,0.05) 0%, transparent 70%)' }} />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 text-center">
        ...
        <motion.h2 initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          style={{ fontFamily: 'Inter, sans-serif', fontWeight: 800, letterSpacing: '-0.03em', fontSize: 'clamp(2.2rem, 5vw, 4.2rem)', color: '#EDE7DC', lineHeight: 1.05, marginBottom: '1.25rem' }}>
          Bereit für eine Webseite,<br />
          <span style={{ color: '#C4A46A' }}>die wirklich verkauft?</span>
        </motion.h2>

        <motion.p initial={{ opacity: 0, y: 12 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.6, delay: 0.2 }}
          style={{ fontFamily: 'Inter, sans-serif', fontSize: '1rem', color: '#7A7468', lineHeight: 1.65, maxWidth: 460, margin: '0 auto 2.5rem' }}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}}
          ...
          style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: '#4A4438' }}>
